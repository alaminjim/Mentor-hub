...
import { 
  ...
  Mail, Link as LinkIcon, ShieldCheck, Heart
} from "lucide-react";
...
export default function SavedEventsPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/dashboard/events/saved").then(r => r.json());
       ...
       <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Interest Vault</span>
             </div>
             <h1 className="text-4xl md:text-6xl font-black tracking-tighter italic lowercase">Saved <span className="text-primary italic">Events.</span></h1>
             <p className="text-muted-foreground text-sm font-medium lowercase">Academic sessions you have prioritized for future reference.</p>
          </div>
       </div>
       ...
       {data.length > 0 ? (
                   ...
                   >
                      ...
                      <div className="space-y-4">
                         <div className="flex items-center justify-between">
                            <div className="px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-[8px] font-black uppercase tracking-widest text-primary flex items-center gap-1.5">
                               <Bookmark className="size-2.5 fill-current" /> Save for Reference
                            </div>
                         </div>
          ...
          <div className="py-24 text-center space-y-6">
             ...
             <div className="space-y-1">
                <h3 className="text-2xl font-black tracking-tighter italic">Empty Vault.</h3>
                <p className="text-muted-foreground text-xs font-medium lowercase">You haven't bookmarked any sessions yet.</p>
             </div>
             <Link href="/events" className="inline-flex h-12 px-8 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest items-center">Browse Calendar</Link>
          </div>
       )}
    </div>
  );
