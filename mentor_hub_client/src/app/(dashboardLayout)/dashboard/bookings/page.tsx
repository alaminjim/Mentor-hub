    ...
    <>
      ...
      <div className="max-w-6xl mx-auto py-12 px-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-8">
          <div>
            <h1 className="text-5xl font-black tracking-tighter lowercase mb-4">
          ...
          <div className="flex gap-4">
             <button className="px-6 py-3 rounded-2xl bg-muted border border-border text-xs font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all">upcoming</button>
             <button className="px-6 py-3 rounded-2xl bg-muted border border-border text-xs font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all text-slate-500">archived</button>
          </div>
        </div>

        {bookings.length === 0 ? (
          <div className="py-32 text-center bg-card rounded-[3rem] border border-border shadow-sm">
             <Calendar className="size-16 text-muted-foreground mx-auto mb-6 opacity-20" />
            ...
            {bookings.map((booking) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-card group rounded-[3rem] p-8 border border-border hover:border-primary/20 transition-all shadow-sm relative overflow-hidden"
                >
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center gap-10">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-6">
                         ...
                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">id: {bookingId.slice(-8)}</span>
                      </div>

                      <h2 className="text-3xl font-black tracking-tighter lowercase mb-8">
                        {booking.subject}
                      </h2>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                        <div className="space-y-2">
                           ...
                           <p className="text-xs font-black text-foreground">{formatDateTime(booking.scheduledAt)}</p>
                        </div>
                        <div className="space-y-2">
                           <p className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-2">
                              <Clock className="size-3" /> Time
                           </p>
                           <p className="text-xs font-black text-foreground">{booking.time || "standard"}</p>
                        </div>
                        <div className="space-y-2">
                           <p className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-2">
                        ...
                        <div className="space-y-2">
                           ...
                           <p className="text-xl font-black text-foreground tracking-tighter">${booking.totalPrice}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row lg:flex-col gap-4 min-w-[240px]">
                       {booking.status === "COMPLETED" ? (
                         ...
                         <button 
                           ...
                           className="w-full flex items-center justify-center gap-3 h-14 bg-primary text-white rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all shadow-xl shadow-primary/20"
                         >
                           Pay & Finalize <DollarSign size={14} />
                         </button>
                       ) : (
                         <button className="w-full flex items-center justify-center gap-3 h-14 bg-muted border border-border hover:bg-primary hover:text-white hover:border-primary text-muted-foreground rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">
